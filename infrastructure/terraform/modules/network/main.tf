...
# default_tags del provider "aws.network" pasado a este módulo (ADR-0006 D25) — ningún
# recurso necesita mezclarlos a mano; solo se añaden aquí los tags específicos del recurso.
...
resource "aws_internet_gateway" "this" {
  ...
  tags = { Name = "runcriticon-${var.environment}-igw" }
}
...
resource "aws_eip" "nat" {
  ...
  tags = { Name = "runcriticon-${var.environment}-nat-eip" }
}
...
resource "aws_nat_gateway" "this" {
  ...
  tags = { Name = "runcriticon-${var.environment}-nat" }
...
resource "aws_route_table" "public" {
  ...
  tags = { Name = "runcriticon-${var.environment}-public-rt" }
}
...
resource "aws_route_table" "private" {
  ...
  tags = { Name = "runcriticon-${var.environment}-private-rt" }
}
...
resource "aws_security_group" "connector" {
  ...
  tags = { Name = "runcriticon-${var.environment}-connector" }
}
...
resource "aws_security_group" "database" {
  ...
  tags = { Name = "runcriticon-${var.environment}-database" }
}
