...
description: >
  ...
  Angular Material 3 con tokens, standalone OnPush, textos con i18n, accesibilidad y tests.
...
Traduce una maqueta hi-fi de `docs/diseno/` a componente(s) Angular siguiendo las convenciones de `frontend-feature-dev`. La maqueta manda en **layout, jerarquía y contenido**; Material 3 manda en **componentes, theming y accesibilidad**. No se copia el CSS de la maqueta: se reproduce su diseño con los medios del proyecto.
...
- **Componentes identificados** → su equivalente Material (tabla de mapeo abajo).
...
### 3. Mapeo maqueta → Material

| En la maqueta | En el componente |
|---------------|------------------|
| Botones | `MatButtonModule` (variante según jerarquía visual: filled/tonal/outlined/text) |
| Cards | `MatCardModule` |
| Inputs, selects, textareas | `MatFormFieldModule` + `MatInputModule`/`MatSelectModule` + ReactiveForms tipados |
| Tablas / listados densos | `MatTableModule` o `MatListModule` según densidad |
| Chips / tags | `MatChipsModule` (los tags del dominio son chips) |
| Modales | `MatDialog` con focus management (foco al primer control, devolución al disparador) |
| Fechas | `MatDatepickerModule` |
| Toggles, checkboxes | `MatSlideToggleModule` / `MatCheckboxModule` |
...
- **Colores, espaciado, tipografía**: NUNCA copiar valores hex/px de la maqueta. Usar tokens M3 (`var(--mat-sys-*)`) y la escala del theme. Si la maqueta usa un color que el theme no tiene, es una conversación de theming (`_theme.scss`), no un hardcode en el componente.
- **Estructura**: SCSS con ámbito de componente; grid/flex propios solo para el layout de página; `::ng-deep` prohibido.
...
- Reproducir la maqueta con `<div>` estilizados cuando existe el componente Material equivalente (se pierde la a11y de serie).
...
- ADR-0012 D1-D8 (Material, theming, a11y), D20 (skeleton/optimistic UI)
