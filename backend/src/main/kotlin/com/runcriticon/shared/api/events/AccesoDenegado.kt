 ...
 * crear una dependencia impuesta al otro. Vivió primero en `auditoria.api.events` (al autorizar la publicación de un plan a un grupo): funcionó
 * mientras solo `planificacion` lo publicaba, pero `auditoria` ya depende de `identidad` (anonimización de sus asientos de auditoría al ejercer el derecho de supresión) — el día que `identidad` también necesitó publicarlo, `identidad → auditoria` +
