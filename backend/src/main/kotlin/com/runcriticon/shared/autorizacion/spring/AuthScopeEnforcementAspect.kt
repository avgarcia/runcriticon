 ...
 * (ADR-0009 D11). **No inyecta el filtro** — el propio método ya recibe `clubId` y lo aplica en su
 * query (D4); este aspecto es la red de seguridad que falla cerrado si esa firma se rompe o se
 * invoca con datos de otro club.
 *
 * Scopes distintos de `CLUB` (`OWNED`, `GRUPOS_DEL_ENTRENADOR`, `MIS_GRUPOS`) no tienen todavía una
 * verificación equivalente implementada: el aspecto falla cerrado en cuanto los detecta, para que
 * un `@AuthScope` declarado nunca quede sin nadie que lo haga cumplir.
 *
 * Riesgo conocido: solo actúa sobre llamadas que pasan por el proxy Spring (bean del repositorio);
 * una auto-invocación interna del mismo bean lo esquiva. La anotación debe seguir en la clase que
 * implementa el método (el impl del `@Repository`), no en el puerto — Spring AOP no hereda
 * anotaciones de método declaradas en la interfaz.
 */
@Aspect
@Component
