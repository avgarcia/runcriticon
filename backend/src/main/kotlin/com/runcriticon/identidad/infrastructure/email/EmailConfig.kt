...
/**
 * Configuración del envío de emails, mapeada desde el prefijo `runcriticon.email` (ADR-0013).
 * Los valores se definen por entorno en `application.yml`; el secreto `postmark.apiKey` llega como
 * variable de entorno desde SSM `SecureString`, nunca embebido en el código.
 *
...
/**
 * Registra [EmailConfig] como bean de propiedades. Aislada en su propia clase para no obligar a
 * anotar la `data class` con `@Component`/`@ConfigurationPropertiesScan`.
 */
@Configuration
