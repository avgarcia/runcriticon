...
/**
 * Advice global de errores de framework (LAL-58): JSON malformado, tipos inválidos en
 * path/query, rutas o métodos inexistentes y cualquier excepción no controlada se traducen al
 * contrato `{code, field?, message}` de ADR-0012 D19 con mensajes neutros — el detalle de la
 * excepción va al log, nunca al body. Los errores de dominio NO pasan por aquí: cada controller
 * mapea su `Either` con [toErrorResponse].
 *
 * Vive en `identidad.infrastructure.rest` porque el modelo [ErrorResponse] se genera en este
 * paquete (openApiGenerate en build.gradle.kts) y `shared` no puede depender de `identidad`.
 * Cuando otro módulo tenga controllers, mover el modelo generado y este advice a un paquete neutro.
 */
...
class GlobalRestExceptionHandler {
    ...
    fun onUnexpected(ex: Exception): ResponseEntity<ErrorResponse> {
        // Las excepciones de seguridad las traduce el filter chain (401/403), no este advice.
    ...
    private fun respond(
        ...
        ResponseEntity.status(status).body(ErrorResponse(code = code, message = message, field = field))
}
