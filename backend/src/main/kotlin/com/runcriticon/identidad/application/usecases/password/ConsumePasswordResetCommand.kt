...
/**
 * Consumo del reseteo de contraseña. Es **público y anónimo**: el usuario se autentica con el token del email, no con
 * la matriz de autorización — por eso, como [ConsumeMagicLinkCommand] y [ActivateAccountCommand], NO la consulta y el
 * endpoint que lo expone se marca [com.runcriticon.shared.autorizacion.annotations.NoAuthRequired].
 *
 * Verifica y consume el magic link de propósito `RESETEO` (aislamiento de propósito, un solo uso, caducidad 15 min,
 * token timing-safe), exige que la cuenta esté `ACTIVO`, valida la nueva contraseña ([PasswordPolicy]), fija el hash,
 * registra el histórico y **invalida todas las sesiones activas del usuario** ([SessionRevoker], D8: una sesión robada
 * no sobrevive al reseteo). Deja asiento de auditoría `PASSWORD_CAMBIADA` y devuelve el [Principal] que la capa api
 * guardará en la sesión (auto-login). Todo en una transacción.
 *
 * Funciona también para un usuario solo-magic-link que aún no tenga contraseña: fija su primera
 * ([com.runcriticon.identidad.domain.user.User.changePassword] solo exige estado `ACTIVO`, no un hash previo).
 */
@ApplicationService
@NoAuthRequired("Consumo de reseteo: el usuario se autentica con el token del email (ADR-0003 D8)")
class ConsumePasswordResetCommand(
