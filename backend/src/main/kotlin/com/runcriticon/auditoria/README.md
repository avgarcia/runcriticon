...
Arrancó con la asignación de entrenadores a grupos y su autorización de publicación: `PublishPlanCommand` (`planificacion`) fue la
...
**Productores de `AccesoDenegado` hoy** (tras extender su emisión al resto de casos de uso):
...
`@AuditAccess`. Hoy el único es `seguimiento.ListCoachAlertsQuery` (panel de alertas del entrenador): el entrenador leyendo reportes
de salud de sus alumnos, un evento por alumno con alerta activa.

**No cubre editar los tags de un alumno sin alterar los planes ya publicados**: los tags de un alumno no son "datos de salud" según el alcance explícito de D15
(marcas, sesiones, lesiones, observaciones médicas). Ese requisito se resolvió con una auditoría local propia en
...
mecanismo de ADR-0017). Detalle en `RGPD.md`.
...
hasta que `identidad` necesitó publicar `AccesoDenegado` (al extender su emisión al resto de casos de uso) y formó un ciclo con la dependencia inversa
...
| `AdminEliminado` v1 | `identidad` | Igual que arriba, solo `actor_id` (un admin nunca es `sujeto_id`) | `AuditTrailAnonymizationListener` |
