...
|---|---|---|
| `runcriticon.club-taxonomia.retention.cron` | `0 0 3 * * *` | Cron del job de retención (`ClubTaxonomiaRetentionJob`) que purga `persona_eliminada` y `evento_procesado`. Desfasado 15 min de `auditoria` para no competir por conexión a la misma hora (ver comentario en `application.yml`) |
