...
| `StudentDeletionListenerTest`: reentregar la misma baja no vuelve a borrar | Unitario (idempotencia de `@ApplicationModuleListener`, fake de `ProcessedEventTracker`) | Con el outbox reintentando eventos, un reproceso no idempotente podría lanzar errores o corromper el estado de la proyección local del alumno |
| `StudentDeletionListenerTest`: la baja de un admin no borra la proyección pero sí anonimiza su `actor_id` en auditoría | Unitario (RGPD) | Si se confunde el tratamiento de un admin con el de un alumno, se pierde traza de auditoría necesaria o se deja PII sin anonimizar |
| `MergeSuggestionListenerTest`: reentregar el mismo evento no vuelve a recalcular | Unitario (idempotencia, fake de `ProcessedEventTracker`) | Reprocesar el mismo evento podría duplicar o inflar sugerencias de fusión de grupos, generando ruido operativo para los entrenadores |
