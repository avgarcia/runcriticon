...
class AssignCoachToGroupCommandTest :
    FunSpec({
        ...
        test("asigna al entrenador y devuelve la lista recalculada") {
            val repository = groups()

            val result =
                AssignCoachToGroupCommand(repository, InMemoryCoachLookup(setOf(entrenador)))
        ...
        test("repetir la asignacion es idempotente") {
            val repository = groups()
            val command = AssignCoachToGroupCommand(repository, InMemoryCoachLookup(setOf(entrenador)))
        ...
        test("un grupo que no existe da GroupNotFound y no toca al entrenador") {
            ...
            AssignCoachToGroupCommand(repository, lookup)
        ...
        test("un id que no es entrenador da CoachNotFound y no escribe la asignacion") {
            ...
            AssignCoachToGroupCommand(repository, InMemoryCoachLookup(emptySet()))
