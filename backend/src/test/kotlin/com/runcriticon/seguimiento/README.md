...
| `SeguimientoDeletionListenerTest`: reentregar el mismo evento no vuelve a borrar | Unitario (idempotencia RGPD, fake de `ProcessedEventTracker`) | Un reintento del outbox tras la baja de un alumno podría fallar o duplicar el borrado de su proyección de seguimiento |
| `SeguimientoDeletionListenerTest`: un entrenador eliminado borra sus filas de `grupo_entrenador` | Unitario (RGPD) | Si no se limpia la relación entrenador-grupo al eliminar al entrenador, quedan referencias huérfanas que rompen consultas de alertas y carga de trabajo |
