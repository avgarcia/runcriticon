...
- **`Result.Forbidden`** — variante de `DomainError` que devuelven los servicios de autorización al denegar (incluida la razón `proyeccionStale`). El adaptador REST la traduce a HTTP 403 con cuerpo neutro (ADR-0009 D12).
...
- **RAT** (Registro de Actividades de Tratamiento, Art. 30) — documento versionado en `docs/legal/rat.md` que enumera los tratamientos, finalidades, categorías de datos, destinatarios, transferencias y plazos. Obligatorio por tratar datos de salud (ADR-0014 D19).
...
- **`Result<T, DomainError>`** — patrón para devolver errores como **valor**, no como excepción. Los errores de dominio (validación, autorización, conflicto) cruzan capas como `Result`; las excepciones quedan para fallos del framework (ADR-0008 D11).
- **`DomainError`** — tipo sellado que enumera los errores de dominio (`Forbidden`, `NotFound`, `InvalidInput`, `Conflict`, `ProjectionStale`...). El adaptador REST lo traduce a códigos HTTP estables (ADR-0008 D11).
