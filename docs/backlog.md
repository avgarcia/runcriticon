...
| M3b | Ver y filtrar los alumnos del club | El admin ve la lista de alumnos del club con su estado (invitado / activo) y sus tags como chips, y la filtra por tags para localizar a quién clasificar o revisar. El entrenador ve la misma lista limitada a los alumnos que le corresponden. Es la pantalla sobre la que se opera M5 y M9. |
| M3c | Ver los entrenadores del club y su carga | El admin ve los entrenadores del club, su estado y los grupos que llevan con cuántos alumnos suman, para repartir el trabajo y detectar quién está sin grupo asignado. Es la base de M8. |
...
- 2026-09-04 — **Alcance de M15 recortado a lo que entrega M17**. La vista de cumplimiento explícita del entrenador por grupo (heatmap hecho/parcial/no hecho/sin reportar) se aplaza conscientemente: se priorizó implementar M17 (panel de alertas por excepción, LAL-116, PR #397) porque los eventos que la alimentarían ya se publicaban sin consumidor, y M15 se considera cubierta por M17 hasta que la beta con el club piloto demuestre lo contrario. `docs/research/findings.md` (P2) ya advertía que ambas cubren necesidades distintas — el aplazamiento no es un descarte silencioso. Disparador de reapertura y detalle técnico en ADR-0015 §A4. Total MUST: 21 (sin cambios).
