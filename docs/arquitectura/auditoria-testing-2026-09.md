...
| P2-5 | Esperas fijas: 15 `SETTLE_MILLIS` y ~25 bucles de sondeo hechos a mano | Migrar a `Scenario` de Spring Modulith o a Awaitility (ambos ya en el classpath) | LAL-170 |
...
| P2-8 | ADR-0010 promete y no existe: PITest nocturno (D9, el ADR ya lo reconoce), retry de Gradle y `retries: 1` de Playwright (hoy 2, D21), `dependency-review` (D14), `format:check` de Prettier (D7), cobertura publicada (D7), dataset de staging (§9, ADR-0006 D21), `loadTest` en ningún workflow | Decidir por punto: implementar o enmendar el ADR | PITest: LAL-172; resto: **nuevo** |
