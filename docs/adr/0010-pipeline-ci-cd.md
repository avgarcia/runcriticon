...
- **Estado**: Aceptado
- **Fecha**: 2026-05-22 · revisado 2026-05-29 (reorganización Nivel 1 + cierre completo del ciclo del pipeline: índice + premisas heredadas + NFRs anclados al plan Free de GitHub Actions + numeración de sub-decisiones D1-D23 con anchors estables; nuevas sub-decisiones D13-D23 sobre umbrales de cobertura, catálogo unificado de tests críticos, caché, triggers por path, concurrencia, versionado de imágenes Docker, reproducibilidad, política de PRs, tests flaky, observabilidad básica y procedimiento operativo de rollback; tres notas finales sobre SemVer, feature flags y revisión periódica) · revisado 2026-06-15 (premisa heredada de ADR-0001 D12: Spring Boot 3.x → 4.x; no cambia ninguna sub-decisión del pipeline) · **revisado 2026-07-11** (D19 — se activa `dependencyLocking` en el backend (`gradle.lockfile`/`settings-gradle.lockfile` commiteados) y se pinean por digest SHA256 las 3 imágenes base del `Dockerfile` (`node:26-alpine`, `ghcr.io/graalvm/jdk-community:21`/`:25`), que hasta ahora usaban tags móviles. Se corrigen también los ejemplos de D19 que no coincidían con el stack real: `eclipse-temurin`→`ghcr.io/graalvm/jdk-community` (ADR-0016), se elimina la mención a un stage `nginx` inexistente, `actions/setup-java`→`graalvm/setup-graalvm`) · **revisado 2026-07-11** (D13 — auditoría de deriva doc↔código: no existe Kover en backend ni `coverageThreshold` en frontend; D13 se marca explícitamente como diseño objetivo aplazado, no estado actual, con disparador propio y entrada cruzada en ADR-0015 §CI/CD. Sin cambio de código) · **revisado 2026-09-23** (D19 — el stage `frontend-build` vuelve a `node:22-alpine`: el literal `node:26-alpine` describía un bump de Dependabot que desalineaba la imagen de `frontend/.nvmrc` (22.22.3), contra el propio anclaje «Node con versión fijada en `.nvmrc`» de D19 y contra ADR-0001 D12) · **revisado 2026-09-24** (D8 — `loadTest` y el seed sintético de `staging` se marcan explícitamente "pendiente de implementar", mismo patrón que ya usa D9 para PITest: bloqueados en la práctica porque no hay camino de despliegue a `staging`; D11 — aclara que no promete test automático de compatibilidad hacia atrás y añade nota de pendiente sobre la validación de los ~29 repos JDBC contra el esquema migrado; D21 — corrige "contenedor por test class, no por test method" al patrón real adoptado, el singleton de `IntegrationTestBase` con aislamiento por club propio de test, no por contenedor) · **aceptado 2026-05-29**
...
Esta es una **regla de equipo**, no técnica: la revisión de PR de migraciones (ADR-0004 D9) es donde se verifica. No hay (ni este ADR promete) un test automático de compatibilidad hacia atrás.

**Pendiente de implementar, más allá de lo que ya cubre D8** — `ContextoArrancaTest` valida el esquema migrado solo contra las clases `@Entity` (`ddl-auto: validate`, ~11 entidades); los repositorios JDBC del proyecto (~29, SQL en `const val`) no se validan contra el esquema migrado por ningún test. Mismo patrón honesto que D9: la decisión (migrar todas las Flyway desde cero en cada PR) sigue vigente, falta extender la cobertura a los repos JDBC. Se diseña junto con los guardarraíles de migraciones Flyway que exige ADR-0004 D9 y hoy no existen — mismo problema, una sola solución.
...
- **Lockfiles commiteados y obligatorios**:
  - `backend/gradle.lockfile` + `backend/settings-gradle.lockfile` (Gradle dependency locking, `dependencyLocking { lockAllConfigurations() }` en `build.gradle.kts`). Dependabot (ecosistema `gradle`) **no** regenera el lockfile por su cuenta al abrir un PR — a diferencia de npm/yarn, solo toca las versiones en `build.gradle.kts`/`libs.versions.toml`; tras mergear ese PR (o cambiar una dependencia a mano) hay que regenerar con `./gradlew dependencies --write-locks` en un commit aparte.
  - `frontend/package-lock.json` (npm).
  - `gradle/verification-metadata.xml` opcional para integridad de dependencias — no implementado (no bloquea D19: el lockfile ya fija versiones resueltas, la verificación de checksums es un anclaje adicional, no el mecanismo base).
- **Toolchain bloqueada**:
  - Gradle Wrapper con versión fijada (`gradle/wrapper/gradle-wrapper.properties` + `gradle-wrapper.jar`).
  - Node con versión fijada en `.nvmrc` (leída por `actions/setup-node`).
  - Java con versión exacta (`java-version: '21'`) vía `graalvm/setup-graalvm` (no `actions/setup-java`: GraalVM CE no es una distribución soportada por esa action — ADR-0016 D6).
- **Imágenes Docker base pineadas por SHA** (`backend/Dockerfile`):
  - `FROM node:22-alpine@sha256:<digest>` (stage `frontend-build`). El major de Node **lo fija `frontend/.nvmrc`**, no la imagen: el stage debe usar el mismo major que CI; Dependabot solo refresca el digest (los majors de `node` se ignoran en `.github/dependabot.yml`).
  - `FROM ghcr.io/graalvm/jdk-community:21@sha256:<digest>` (stage `backend-build`, target Java 21 — ADR-0016 D7).
  - `FROM ghcr.io/graalvm/jdk-community:25@sha256:<digest>` (stage `runtime` — ADR-0016 D4). No hay stage nginx: la SPA la sirve la propia app Spring Boot (ADR-0001 D11), no un servidor estático aparte.
  - Dependabot (ecosistema `docker`, directory `/backend`) actualiza los digests con PRs revisables — mismo patrón que las GitHub Actions pineadas por SHA en `ci.yml`.
...
- **Tests de integración con Testcontainers**: **contenedor singleton compartido por toda la JVM de test** (`IntegrationTestBase`, patrón *singleton container* de Testcontainers), no uno por test class — la redacción original de este punto decía "contenedor por test class, no por test method", pero el patrón real que adoptó el proyecto va un nivel más arriba: un solo contenedor para toda la suite, no uno por clase. Razón del cambio: arrancar un Postgres por clase (~35+ clases `@SpringBootTest`) satura el runner de CI en tiempo y memoria; el singleton amortiza ese coste. El aislamiento entre clases no lo da el contenedor, lo da la disciplina de **club propio por test** (`ClubId` nuevo por clase, nunca el de bootstrap) — documentada en `docs/arquitectura/testing-de-modulos.md` §4, con el incidente real que motivó la regla (CI rota por un test que borraba sin filtro de club, PR #331). Un test con contenedor propio (`@Testcontainers` + `@Container`) sigue siendo válido cuando la clase necesita su propio ciclo de vida — no es la norma, es la excepción documentada. Datos sintéticos generados; nada de fixtures compartidos entre clases.
...
- **Política de versionado SemVer para releases formales**: el patrón `v<semver>` de D18 está reservado para releases productivos manuales. En MVP, los deploys a `producción` son por commit en `main` (cadencia continua); **no se emiten `v<semver>` de forma rutinaria**. Un tag `v<semver>` se crea en dos casos: (a) un hito de producto que el equipo quiere preservar como referencia (ej. *"versión que vio el club piloto en su primera sesión"*), o (b) un release que se va a comunicar externamente. Convención: incrementos manuales con criterio (major = breaking change observable por usuarios; minor = funcionalidad nueva; patch = corrección). El versionado **no se automatiza** en MVP — la herramienta (release-please, semantic-release) se evalúa cuando los `v<semver>` se vuelvan rutinarios.
- **Feature flags**: no se introducen en MVP. El flujo de trunk-based + PR de vida corta + entrega continua a `staging` (D4-D5) absorbe la mayoría de los casos. **Disparador concreto para introducirlos**: aparece una funcionalidad que no se puede desplegar a producción de un solo golpe (rollout gradual, A/B test, kill switch para un piloto). Cuando llegue, la elección entre librería ligera (`@togglz/togglz-spring-boot-starter` o equivalente con properties) y SaaS (LaunchDarkly, Unleash) será un ADR aparte. Mientras tanto, código no listo va detrás de un `@ConditionalOnProperty` simple.
- **Revisión periódica del ADR a 6 meses**: cuando el primer módulo lleve seis meses corriendo el pipeline real, se audita la disciplina con tres preguntas concretas — (1) ¿los NFRs del pipeline (tiempo total, frecuencia de fallos) se están cumpliendo en la práctica?; (2) ¿el catálogo de tests críticos (D14) sigue mapeando 1:1 a los ADRs del modelo o ha derivado?; (3) ¿la política de tests flaky (D21) ha reducido el número de tests muteados o ha aumentado? Si alguna respuesta es preocupante, se reabre la disciplina (no necesariamente el ADR) con ajustes focalizados.
- El plan de formación [`docs/formacion/github-actions.md`](../formacion/github-actions.md) acompaña a este ADR.
- **Revisión del 2026-07-11 (D19 — auditoría de deriva doc↔código)**: no existía `gradle.lockfile` ni `dependencyLocking` en el backend, y las 3 imágenes base del `Dockerfile` usaban tags móviles sin pinear por SHA. Se activa `dependencyLocking { lockAllConfigurations() }` (lockfiles generados y commiteados), y se pinean las 3 `FROM` por digest SHA256 real (resuelto contra Docker Hub/GHCR). Verificado: el lockfile efectivamente fuerza la versión resuelta (probado bajando una versión en el catálogo y comprobando que Gradle la ignora y usa la del lock); el `Dockerfile` con digest se valida sintácticamente con BuildKit (el pull local falla por un problema de credential-helper de Docker Desktop ajeno a este cambio — no verificable en este entorno, sí en CI). De paso se corrigen los ejemplos de D19 que citaban un stack ficticio (`eclipse-temurin`, `nginx`, `actions/setup-java`) en vez del real (GraalVM CE, sin nginx, `graalvm/setup-graalvm`).
