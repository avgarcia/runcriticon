...
La especificación **OpenAPI 3.x escrita a mano es la fuente de verdad**. De ella se generan, mediante generadores de código: los *stubs*/interfaces del servidor (Kotlin) y el cliente TypeScript tipado del frontend (Angular). Un **test de contrato** en CI verifica que la implementación real del backend cumple la spec — así la spec no puede derivar de la realidad en silencio, que es el único punto débil del contract-first.
...
- **Generador del backend**: **`openapi-generator`** con el template `kotlin-spring`, invocado como plugin de Gradle antes de compilar Kotlin. Genera interfaces y modelos; la implementación es código a mano que cumple las interfaces.
...
- **Test de contrato**: ya descrito arriba; obligatorio en *verde* para mergear (ADR-0010).
