...
- Los payloads del outbox de Spring Modulith **siguen siendo JSON** en `JSONB` (ADR-0004 D6) — legibles para debugging. El caso de PII pendiente de un alumno ya borrado se resuelve sin reescribir payloads: la política de reintento/DLQ de este ADR (D13) junto con el NFR de propagación `< 24h p95` (`ADR-0014` NFRs) llevan el evento a completado en el caso normal, y la retención a 30 días del outbox (`ADR-0004` D11) purga la fila una vez completada.
...
- Cada personalización tiene su último evento (`PersonalizacionAplicada` si está activa, `PersonalizacionRetirada` si se retiró) → suficiente para reconstruir la sección de personalizaciones del read model.
...
- **Revisión del 2026-09-19**: corrige la cita colgante a la `ADR-0004` D16 retirada en su revisión del 2026-08-26. En la línea ~270, la mención a "la reescritura del flujo RGPD (ADR-0004 D16)" nombraba un mecanismo (reescritura activa de payload al DSAR) que nunca se implementó y que el D16 corregido ya no describe; se sustituye por la razón real por la que el caso de PII pendiente en el outbox no necesita reescritura: la política de reintento/DLQ de este ADR (D13) más el NFR de propagación `< 24h p95` (`ADR-0014` NFRs) completan el evento en el caso normal, y la retención a 30 días del outbox (`ADR-0004` D11) purga la fila. La otra cita señalada (línea ~50, "el borrado (ADR-0004 D16) condicionan la operación de events-first") se revisa y se confirma correcta en términos generales — D16 sigue siendo una restricción real sobre events-first, solo cambió el mecanismo concreto que describe — así que no se toca. Estado se mantiene Aceptado (corrección de *drift*, no nueva decisión), mismo patrón que la revisión de D16 en ADR-0004 (2026-08-26).
- **Revisión del 2026-09-19**: alinea las dos citas restantes al evento de personalización con el nombre implementado — `PersonalizacionAplicada`, no `SesionPersonalizada` (corregido en ADR-0002 D9 en su revisión del 2026-09-19, mismo motivo: `NamingConventionArchTest` de ADR-0008 D4 prohíbe el token `Sesion` en nombres de clase). Líneas ~441 (ejemplo de ordering de D14) y ~505 (reprocesamiento de proyecciones de D15). Estado se mantiene Aceptado.
