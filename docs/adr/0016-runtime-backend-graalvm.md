...
ADR-0001 (D2) fija **Kotlin sobre Spring Boot 4** como backend. Falta decidir **sobre qué runtime corre** ese backend en producción. Hoy la elección por defecto sería OpenJDK / Temurin (HotSpot). En 2026 hay dos alternativas reales con GraalVM, muy distintas entre sí:
...
- **Kotlin sobre Spring Boot 4** (ADR-0001 D2). El lenguaje y el framework no cambian con esta decisión.
...
- **Target de compilación: Java 21** — detekt 1.23.7 y Kotlin 2.1.0 **no soportan `jvm-target 25`** (verificado en CI el 2026-06-03; detekt tope 22). El jar (bytecode 21) corre en el runtime 25 (forward-compatible). Se alinea la compilación a 25 cuando esas herramientas lo soporten (disparador de D5). Upgrade aplicado por el cauce de D5, sin reabrir A = JIT.
  - **Actualización 2026-06-15** (tras subir Kotlin a 2.3.21, ADR-0001 D12): el bloqueante **ya es solo detekt**. Verificado empíricamente en este worktree compilando con `jvm-target 25`: el lado Kotlin (`kaptGenerateStubsKotlin`) lo acepta, y únicamente detekt aborta con `Invalid value (25) passed to --jvm-target, must be one of [1.6, 1.8, 9, …, 21, 22]`. El target sigue fijado en 21 hasta que detekt soporte 25 (entonces se alinea por el cauce de D5).
  - **Actualización 2026-06-16** (commit `e312a22`): detekt sube de 1.23.7 a 2.0.0-alpha.4 (posteriormente 2.0.0-alpha.5), versión testada contra JDK 25 — resuelve el bloqueante descrito arriba. Kotlin sube en paralelo a 2.4.0 (`backend/gradle/libs.versions.toml`). El target de compilación **se mantiene en 21**: la nueva versión de detekt ya soporta `jvm-target 25`, pero el alineamiento a 25 sigue reservado al cauce de revisión de D5, no se hace de forma incidental en un bump de dependencia.
...
- **Versión actual**: runtime **GraalVM CE 25** (Java 25 LTS); **compilación en target Java 21**. El bloqueante original (detekt 1.23.7 / Kotlin 2.1.0 no soportaban jvm-target 25, verificado en CI el 2026-06-03) quedó resuelto con el commit `e312a22` (2026-06-16, detekt → 2.0.0-alpha.4+, Kotlin → 2.4.0, ambos ya testados contra JDK 25). El target se mantiene en 21 de forma deliberada: alinearlo a 25 sigue siendo una decisión del cauce de revisión de D5, no un efecto colateral del bump de dependencias.
