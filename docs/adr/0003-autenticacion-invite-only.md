...
**Spring Session desde el día 1** — aunque el MVP corra con una sola instancia, **se usa la API de Spring Session** desde la primera línea de código, no `HttpSession` ad-hoc del contenedor. Razón: cuando ADR-0006 active más de una instancia y la sesión deba pasar a un almacén compartido, el cambio es de **configuración** (un *bean* del backend de almacenamiento), no de **código**. Implementación real: Spring Session **JDBC** sobre el mismo PostgreSQL desde el día 1 (`@EnableJdbcHttpSession`, `SessionConfig.kt`, migración Flyway `_shared/V202606030001__crea_spring_session.sql`) — no un almacén `Map` en memoria. JDBC ya es un almacén **compartido y persistente**, válido tal cual para multi-instancia (ADR-0006): Redis queda como opción futura solo si el rendimiento de JDBC no basta a la escala real, no como paso obligado del camino de escalado.
