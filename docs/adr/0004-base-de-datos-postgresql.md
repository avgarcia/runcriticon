...
Regla complementaria: **prohibido `UUID.randomUUID()` en código de producción** (genera v4). El proyecto provee un servicio inyectable `UuidGenerator` que produce v7; un test ArchUnit lo enforcea (ver *Estrategia de tests críticos*).
...
| **D8 — tipos** | Cualquier `@Column` con `columnDefinition = "TIMESTAMP"` (sin TZ) → falla regla ArchUnit. Cualquier uso de `UUID.randomUUID()` en el código de producción → falla regla ArchUnit (la única generación legítima es vía el `UuidGenerator` v7 del proyecto). | ArchUnit | Sin el guardarraíl, los timestamps sin TZ y los UUID v4 entran por la puerta de atrás (un PR distraído, una librería que los introduce) y descubrir el problema un año después es caro. |
