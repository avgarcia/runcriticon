...
- **Cumplimiento RGPD (D16)** — mecánica de BD del borrado de datos personales compatible con events-first; la categorización y el tratamiento por categoría los fija ADR-0014 D5/D6.
...
| D16 | [Borrado RGPD: mecánica de BD, categorización remitida a ADR-0014](#d16)                | Estratégica  |
...
- **Cada módulo tiene su propia carpeta de migraciones; el historial es único**. Las migraciones de un módulo viven solo en su carpeta y solo tocan su schema (D4), pero Flyway las aplica todas desde **una única tabla de historial, `public.flyway_schema_history`** (`default-schema: public`, las seis carpetas en `spring.flyway.locations`). Cada módulo crea su propio schema en su primera migración (`crea_esquema_{modulo}`), por eso la configuración no declara `schemas:`. La estructura de carpetas en el repo es:
  ```
  backend/src/main/resources/db/migration/_shared/          ← outbox, Spring Session (schema public)
  ...
  backend/src/main/resources/db/migration/auditoria/
  ```
  **Por qué un historial único y no uno por schema**: el monolito modular (ADR-0007) se despliega como **un único artefacto** con un único paso de migración (ADR-0010), así que no hay ningún escenario en el que un módulo migre sin los demás; un historial por schema obligaría a configurar y ejecutar seis instancias de Flyway sin ganar aislamiento real. El historial único da además un **orden total** de todas las migraciones del sistema, que es lo que se reproduce en CI desde cero y en cada entorno. El aislamiento entre módulos lo garantizan la carpeta y el schema (D4, D12), no la tabla de historial. El coste se asume en la extracción de un módulo (ver *Negativas*).
- **Versionado `V{YYYYMMDD}{NNNN}__{descripcion}.sql`**: fecha de creación + secuencia de 4 dígitos. `V202606030002__crea_usuario.sql`. **La secuencia es global entre módulos, no por carpeta**: al compartir historial, dos ficheros con la misma versión en carpetas distintas chocan igual (Flyway aborta por versión duplicada). Ejemplo real: el 2026-06-02 los cinco `crea_esquema_*` llevan `0001`…`0005`, uno por módulo.
- **Regla operativa al crear una migración**:
  - Comprobar **todas** las carpetas de `db/migration/` para la fecha del día y tomar la siguiente secuencia libre, no solo la carpeta propia.
  - La versión debe ser **mayor que la máxima presente en `main`** en el momento de mergear. Flyway no tiene `out-of-order` activado: una migración con versión menor que la última aplicada queda como *ignored* y la validación al arrancar (`validate-on-migrate`, activa por defecto en Spring Boot) falla en cualquier entorno que ya tenga la versión mayor. Si una PR con migración lleva días abierta y `main` ha avanzado, se **renombra** la migración al rebasar. El test de CI desde cero **no** detecta este caso (aplica todo en orden sobre un esquema vacío); solo aparece en `staging`/producción, por eso es regla de revisión.
...
<a id="d16"></a>
### D16 — Borrado RGPD: mecánica de BD, categorización remitida a ADR-0014

**Corregido 2026-08-26 (LAL-127).** La categorización de datos en seis categorías y el tratamiento de cada una al ejercer el derecho al olvido **los fija `ADR-0014` D5/D6** — no este ADR. La versión anterior de D16 fijaba su propia categorización (PII física / derivado anonimizado) que contradecía a D5 en las tres tablas que nombraba, y describía un mecanismo DSAR (`BorradoAlumnoSolicitado`/`BorradoAlumnoCompletado`/`BorradoAlumnoConsumado`, `anonimoId`) que nunca se construyó. El código ya sigue `ADR-0014` desde antes de esta corrección (`PlanificacionErasureJdbc`, `SeguimientoErasureJdbc`); este ADR simplemente deja de afirmar lo contrario. Detalle de la corrección en Notas.

Lo que **sí** decide este ADR — la mecánica de BD necesaria para que D6 de `ADR-0014` se ejecute contra esta topología de schema-por-módulo (D4) y events-first (D11):

- **Mecanismo de propagación**: el módulo `Identidad` emite `AlumnoEliminado(usuarioId, clubId, occurredAt)` (`ADR-0014` D7) — un único evento, sin solicitud/confirmación separadas, sin `anonimoId`. Cada módulo con proyección local del alumno lo consume en su propio `DeletionListener` (`@ApplicationModuleListener`, idempotente por D9 de ADR-0007) y ejecuta su rutina de borrado en su schema, según la categoría que le corresponda por D5/D6 de `ADR-0014`:
  - Categoría 1 (PII primaria) → `DELETE` físico.
  - Categorías 2-3 (auditoría local / de autorización) → anonimización (`actor_id`/`sujeto_id` a `NULL`, IP truncada).
- **Orden de los `DELETE` dentro de un módulo**: ninguna tabla del esquema usa `ON DELETE CASCADE` (decisión implícita en D4, ahora explícita) — el propio `DeletionListener`/`XxxErasureJdbc` de cada módulo ordena sus sentencias para no violar una FK propia del schema (p. ej. sesiones/personalizaciones antes que el plan que las contiene).
- **Categoría 4 (outbox, `event_publication`)**: sin paso activo nuevo. Un evento aún pendiente (`completion_date` nulo) que referencia a un alumno ya borrado se resuelve por los mecanismos que **ya existen**: la política de reintento/DLQ de `ADR-0007` D13 (5 intentos) junto con el NFR de propagación `< 24h p95` (`ADR-0014` NFRs) lo llevan a completado en el caso normal; una vez completado, la retención de 30 días de este mismo ADR (D11) borra físicamente la fila. No hace falta reescribir el payload ni ningún mecanismo adicional — D11 ya cubre este caso con un margen (30 días) más generoso que el NFR de propagación.
- **Plazo operativo**: `< 24h p95` desde `AlumnoEliminado` hasta que todas las proyecciones lo reflejan (`ADR-0014` NFRs), dentro del plazo legal de 1 mes de D11 de `ADR-0014` (Art. 12.3).
...
- **Solo anonimización sin borrado físico de PII**: descartado para la categoría 1. La PII en `identidad.usuario` o las marcas privadas en `seguimiento.marca_alumno` (D7 de ADR-0002 → borrado físico obligatorio) deben desaparecer realmente; mantenerlas anonimizadas en la misma tabla es indistinguible de "no borrar".
- **Soft-delete con `borrado_en TIMESTAMPTZ`** para datos personales: descartado (RGPD pide borrado efectivo, no marca lógica accesible al admin) — sí se usa puntualmente para `TagKey`/`TagValue` por motivo distinto (ADR-0002 D10), no como mecanismo RGPD.
...
| **D9 — migraciones** | CI levanta PostgreSQL en Testcontainers y aplica **todas** las migraciones del proyecto desde cero en cada PR; si una falla, el PR no merge (`ContextoArrancaTest`). Test propio que verifica que cada `V__` nueva contiene un comentario `-- Rollback:` al final del fichero (puede ser "sin rollback automático — restaurar desde backup") — **no existe todavía**: pendiente, no confundir con implementado (y hoy ninguna migración lleva ese comentario). | Integración con Testcontainers + test propio (pendiente) | Una migración rota llega a producción y la siguiente release la sigue sin que nadie lo note hasta que toca restaurar de cero o levantar un entorno nuevo. El rollback documentado es la diferencia entre "lo sabemos" y "lo improvisamos a las 3 AM". |
...
| **D16 — borrado RGPD** | Test de integración del flujo completo: dado un alumno con datos en los módulos con proyección suya, publicar `AlumnoEliminado` → verificar que cada `DeletionListener` deja su schema conforme a la categoría de `ADR-0014` D5/D6 (categoría 1 → filas físicamente borradas; categorías 2-3 → `actor_id`/`sujeto_id` anonimizados). Test de idempotencia: reprocesar el mismo evento no falla ni duplica efecto. | Integración con Testcontainers | Borrado parcial deja residuos de PII en una tabla cualquiera = incidente RGPD reportable. Un listener no idempotente puede fallar en un reintento del outbox y dejar el borrado a medias. La conformidad la verifica un test, no la buena intención del equipo. |
...
- El historial Flyway único (D9) acopla el versionado de todos los módulos: la secuencia de versiones es global y extraer un módulo a su propia base de datos exige partir su historial (nuevo `flyway_schema_history` con *baseline* en la última versión aplicada de ese módulo). Se asume: la extracción no está en el horizonte del MVP y el trabajo está acotado a ese momento.
...
- **Revisión del 2026-08-26 (D16, LAL-127)**: corrige la contradicción entre D16 y `ADR-0014` D5/D6 detectada al construir `seguimiento.reporte_sesion` (LAL-30) — dos de las tres tablas que D16 pedía anonimizar (`personalizacion`, `plan_snapshot_alumno`) ya se borraban físicamente desde antes (`PlanificacionErasureJdbc`), y la tercera (`reporte_sesion`) se implementó también con borrado físico siguiendo el mismo criterio, documentando la contradicción en `seguimiento/RGPD.md` en vez de corregirla inline (regla del corpus: un cambio de ADR va en su propia PR). D16 deja de fijar su propia categorización — remite a D5/D6 de `ADR-0014`, que ya la gobiernan — y retira el mecanismo DSAR (`BorradoAlumnoSolicitado`/`Completado`/`Consumado`, `anonimoId`) descrito en la versión anterior, que nunca se implementó; el mecanismo real es `AlumnoEliminado` + `DeletionListener` por módulo (`ADR-0014` D7). El hueco de PII en eventos de outbox pendientes de un alumno borrado (antes "resuelto" por una reescritura de payload que tampoco existía) se resuelve citando mecanismos que sí existen: `ADR-0007` D13 (reintento/DLQ) + el NFR `< 24h p95` de propagación + la retención de 30 días de este mismo ADR (D11) — sin necesidad de mecanismo nuevo. Estado se mantiene Aceptado (corrección de *drift*, no nueva decisión arquitectónica), mismo patrón que las tres revisiones de 2026-07-11 anteriores. Sigue pendiente, fuera de esta PR por bloqueo del hook `bloqueo-adr-aceptado.sh` (edita `0004`, no `0007`): dos citas a la D16 retirada en `docs/adr/0007-monolito-modular.md` (líneas ~50 y ~270) — ticket de seguimiento abierto para una `feature/revision-adr-0007` futura.
- **Revisión del 2026-09-23 (D9)**: alinea D9 con la configuración real de Flyway, que nunca correspondió a lo escrito: (1) no hay una tabla de historial por schema — hay **un único `public.flyway_schema_history`** (`application.yml`: `default-schema: public`, seis carpetas en `locations`, sin `schemas:`), y la afirmación de que las migraciones de un módulo "no son visibles ni aplicables" desde otro se retira; (2) el formato de versión real es `V{YYYYMMDD}{NNNN}` (fecha + secuencia de 4 dígitos, las 49 migraciones existentes lo siguen), no el `V<timestamp>` de 14 dígitos del ejemplo; (3) la afirmación de que el timestamp "evita colisiones" entre PRs paralelas era falsa con historial único — la secuencia es global entre módulos y se añade la regla operativa (comprobar todas las carpetas del día, versión mayor que la máxima de `main`, renombrar al rebasar). Se añade el rationale del historial único (un único deploy, orden total) y su coste en *Negativas* (partir el historial al extraer un módulo). De paso se marcan como **no implementados** dos guardarraíles que D9 daba por existentes: el validador de nombre de fichero y el test del comentario `-- Rollback:` (ninguna de las 49 migraciones lo lleva) — mismo tratamiento que la revisión D8 del 2026-07-11; la política de rollback documentado se mantiene como decisión. Estado se mantiene Aceptado (corrección de *drift*, no nueva decisión arquitectónica). Detectado al corregir `persistencia.md` en la PR #461.
