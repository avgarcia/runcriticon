...
- **Estado**: Aceptado
- **Fecha**: 2026-05-20 · revisado 2026-05-29 (reorganización Nivel 1: premisas heredadas, NFRs propios, sub-decisiones numeradas D1-D29 con anchors; incorporación de: dimensionado inicial concreto de App Runner y RDS, autoescalado con rangos, política de actualización de PostgreSQL, disparador para Multi-AZ, topología de red con VPC + subnets privadas + VPC connector, acceso administrativo a RDS vía SSM Session Manager sin bastión permanente, dominio propio + estrategia multi-club, CloudFront fuera de MVP con disparador, backend de estado de Terraform en S3 + DynamoDB lock, datos sintéticos en `staging`, convención de tagging obligatoria, alertas de facturación con umbrales, observabilidad sobre CloudWatch en MVP, RTO/RPO objetivo) · **aceptado 2026-05-29** · revisado 2026-07-21 (D30 — el club deja de ser una constante de configuración y pasa a ser una entidad con ficha editable, propiedad del módulo `identidad`) · revisado 2026-07-21 (D30 — decae la razón nº 2 tras la revisión de ADR-0005 D5: `identidad` ya no necesita el nombre del club para el remitente del email; se explicita la FK `usuario.club_id → club.id` como razón de peso y se corrige la atribución de la semilla del club) · revisado 2026-09-19 (D30 — cierra la evaluación de mover la ficha del club a `club_taxonomia`: con `club_taxonomia` ya con código real, se confirma que el club se queda en `identidad`; la FK real es razón suficiente por sí sola) · revisado 2026-09-19 (D4 — corrige la contradicción con ADR-0003 D10: D4 describía sesión en memoria en MVP con migración obligatoria a Redis al subir `min`, pero D10 ya fijó Spring Session JDBC desde el día 1, confirmado por el código real (`store-type: jdbc`, migración `V202606030001__crea_spring_session.sql`); Redis pasa a ser disparador de rendimiento, no paso obligado del escalado — alineado con ADR-0015 tabla maestra "Spring Session en Redis")
...
**Importante**: la sesión ya está resuelta para multi-instancia desde el día 1 — **Spring Session JDBC sobre el mismo PostgreSQL** (ADR-0003 D10), no un almacén en memoria. Subir `min` a 2 o más no exige ningún cambio de sesión: JDBC ya es un almacén compartido y persistente, válido tal cual. **Disparador para migrar a ElastiCache Redis**: solo si el rendimiento de JDBC no basta a la escala real (ADR-0015 A2 / tabla maestra "Spring Session en Redis") — no un paso obligado del escalado de App Runner.
...
- **Pull request obligatoria** para cambios en infraestructura (ADR-0010): el job `terraform` de CI valida formato, sintaxis y tests con provider mockeado (`fmt -check`, `validate`, `test` — sin credenciales AWS, D25). El `terraform apply` real lo ejecuta un operador admin con SSO desde su terminal local (D27) — la pipeline **no** aplica infraestructura automáticamente en el MVP.
...
- **OIDC desde GitHub Actions**: rol específico `github-actions-runcriticon-${entorno}` (nombre con sufijo de entorno) con confianza federada en el OIDC provider de GitHub, restringido por `repo:avgarcia/runcriticon:*`.
- **Permisos mínimos del rol, acotados a despliegue — sin `terraform apply`**: push a ECR, `apprunner:StartDeployment`/`UpdateService`/`DescribeService`/`ListOperations`/`ListServices`, lectura de SSM (`ssm:GetParameter*`), `kms:Decrypt` y `iam:PassRole` acotado a los roles de App Runner. El `terraform apply` de infraestructura lo ejecuta un **operador admin con SSO** desde su terminal local (D18), no el rol de CI — separación deliberada: un token de GitHub Actions comprometido no puede reaprovisionar infraestructura.
...
2. ~~**`identidad` necesita el nombre del club**~~ — **razón decaída el 2026-07-21**. Se apoyaba en que ADR-0005 D5 exigía el remitente visible *"Club Atletismo X (vía Runcriticon)"* desde un adaptador que vive en `identidad/infrastructure/email/`. Esa exigencia se retiró: D5 revisado fija el nombre visible en **solo el producto**, que es lo que el código siempre hizo (`from-name: Runcriticon`, constante de `application.yml`). Ningún componente de `identidad` lee hoy el nombre del club. Se conserva tachada, y no renumerada, para que quien lea D30 vea que el argumento existió y por qué ya no aplica.
3. **La FK real `usuario.club_id → club.id`.** ADR-0004 prohíbe FK entre esquemas, así que la integridad referencial entre usuario y club **solo es posible con ambas tablas en `identidad`**. Con el club en `club_taxonomia`, `usuario.club_id` vuelve a ser un UUID suelto sin garantía de que apunte a un club existente. Con la razón nº 2 decaída, esta es la razón técnica de peso junto a la nº 1.
4. **El club nace en el arranque de `identidad`**: la fila la siembra la migración `V202607210001` y `ClubBootstrapValidator` verifica al arrancar que existe la de `runcriticon.bootstrap.club-id`. (Redacción anterior: *"el comando de semilla que lo crea es `IdentidadSeeder`"* — impreciso: `IdentidadSeeder` crea el **primer admin**, no el club.)
5. **El núcleo compartido queda descartado**: `shared` contiene exclusivamente contratos, no entidades con tabla.
...
- **Revisión del 2026-09-19 (D30)**: cierra la evaluación abierta cuando decayó la razón nº 2 (2026-07-21) — ¿compensa mover la ficha del club a `club_taxonomia` ahora que solo queda la fricción de nombres como coste de mantenerla en `identidad`? El primer disparador previsto (`club_taxonomia` recibe su primer código real) ya se cumplió: el módulo tiene tags, grupos, membresías y varias funcionalidades `must` de fase 1 cerradas. Aun así, **se decide no mover el club**: la razón nº 3 (FK real `usuario.club_id → club.id`, solo posible dentro del mismo esquema por ADR-0004) sostiene la decisión por sí sola, con independencia de la nº 2. Mover el club supondría perder esa integridad referencial, reescribir la migración `V202607210001` y `ClubBootstrapValidator`, a cambio solo de coherencia de nombres — coste que no compensa. La fricción de nombres (D30, Consecuencias negativas) se asume como definitiva, no transitoria. El disparador que sigue vigente para reabrir esta decisión es el **segundo**: multi-club (D16), cuando el club como *tenant* importe de verdad y haya enrutado por subdominio.
