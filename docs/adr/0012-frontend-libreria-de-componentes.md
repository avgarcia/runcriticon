...
- **Estado**: Aceptado
- **Fecha**: 2026-05-22 · revisado 2026-05-30 (reorganización Nivel 1: premisas heredadas, NFRs propios, sub-decisiones numeradas D1-D22 con anchors; incorporación de: **gestión de estado con Angular Signals**, **cliente HTTP generado desde OpenAPI**, **interceptores CSRF/errores/auth**, **estructura por features con lazy loading**, **testing Jest + Playwright + axe-core**, **i18n con `$localize`**, **nivel WCAG 2.1 AA con tests automáticos**, **bundle budget**, **autorización en UI con `/me/permissions`**, **manejo estructurado de errores 4xx**, **theming Material 3 con tokens**, **skeleton screens**) · **aceptado 2026-05-30** · **revisado 2026-07-10** (D1: Angular Material → **spartan.ng**; D3: tokens Material 3 → **variables CSS estilo shadcn**; D4: estilado primario con **utilidades Tailwind en template**; D5: de "sin Tailwind" a **Tailwind v4 como paradigma único** — ver [Opciones consideradas — revisión 2026-07](#opciones-consideradas--revision-2026-07)) · **revisado 2026-07-11** (D9 — ningún componente ya construido usaba `i18n`/`$localize`; se retrofitea el marcado en los 12 componentes existentes, se instala `@angular/localize` (ausente hasta ahora) y se añade `@angular-eslint/template/i18n` para que la regla no vuelva a romperse. Verificado con `ng extract-i18n` real: 157 mensajes)
...
**Enforcement**: `@angular-eslint/template/i18n` (`checkText: true`, `checkAttributes: true`, `checkId: false` — Angular genera el ID por hash del contenido, exigir IDs manuales a mano no aporta en el MVP) bloquea en `npm run lint` cualquier texto o atributo textual nuevo sin marcar. Verificado end-to-end con `ng extract-i18n`: 157 mensajes reales extraídos de los componentes existentes (login, activate, home, coaches, alumnos, magic-link-*, password-reset-*, force-password-change). Requiere `@angular/localize` como dependencia (`polyfills` en `angular.json` y `setup-jest.ts` para los tests unitarios).
