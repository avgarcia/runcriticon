...
- [Decisión](#decision)
  - **Modelo y capas**
    - [D1 — Tres capas: RBAC + nivel de objeto + `club_id`](#d1)
    - [D2 — RBAC con Spring Security en el adaptador de entrada](#d2)
    - [D3 — Nivel de objeto en la capa de aplicación](#d3)
    - [D4 — Filtrado por `club_id` sistemático en los repositorios](#d4)
  - **Topología y reglas**
    - [D5 — Autorización por módulo; sin módulo central de autorización](#d5)
    - [D6 — Núcleo compartido: principal + primitivas + matriz fija](#d6)
    - [D7 — Servicio de autorización por módulo](#d7)
    - [D8 — Proyecciones locales de relaciones alimentadas por eventos](#d8)
    - [D9 — Política frente a proyección stale: fail-closed con timeout](#d9)
  - **Patrón de listados y errores**
    - [D10 — Listados filtrados en query, nunca en memoria](#d10)
    - [D11 — Aspecto `@AuthScope` que inyecta filtros en las queries](#d11)
    - [D12 — Errores de autorización como `Result.Forbidden`](#d12)
  - **Garantías arquitectónicas**
    - [D13 — ArchUnit obligatorio: todo `@ApplicationService` autoriza](#d13)
    - [D14 — Tests de acceso cruzado obligatorios por caso de uso](#d14)
  - **Auditoría**
    - [D15 — Alcance: denegaciones siempre, accesos a salud y perfil personal](#d15)
    - [D16 — Emisión asíncrona vía evento + outbox](#d16)
    - [D17 — Módulo `auditoria` dedicado, esquema propio](#d17)
  - **UX y operación**
    - [D18 — Endpoint `GET /me/permissions` como ayuda de UX, no barrera](#d18)
    - [D19 — Sin rol de soporte interno en MVP (aplazamiento consciente)](#d19)
- [Matriz de visibilidad](#matriz-de-visibilidad)
...
- **Datos de salud sujetos a RGPD** (ADR-0014). Justifica la auditoría y el borrado mixto al ejercer el derecho al olvido (ADR-0014).
...
**Opción A: RBAC + autorización a nivel de objeto, aplicadas en tres capas.** La decisión se desglosa en **diecinueve sub-decisiones** agrupadas en seis bloques: modelo y capas, topología y reglas, patrón de listados y errores, garantías arquitectónicas, auditoría, y UX/operación.

### Modelo y capas

#### <a id="d1"></a>D1 — Tres capas: RBAC + nivel de objeto + `club_id`
...
#### <a id="d2"></a>D2 — RBAC con Spring Security en el adaptador de entrada
...
#### <a id="d3"></a>D3 — Nivel de objeto en la capa de aplicación
...
#### <a id="d4"></a>D4 — Filtrado por `club_id` sistemático en los repositorios
...
#### <a id="d5"></a>D5 — Autorización por módulo; sin módulo central de autorización
...
#### <a id="d6"></a>D6 — Núcleo compartido: principal + primitivas + matriz fija
...
#### <a id="d7"></a>D7 — Servicio de autorización por módulo
...
#### <a id="d8"></a>D8 — Proyecciones locales de relaciones alimentadas por eventos
...
#### <a id="d9"></a>D9 — Política frente a proyección stale: fail-closed con timeout
...
#### <a id="d10"></a>D10 — Listados filtrados en query, nunca en memoria
...
#### <a id="d11"></a>D11 — Aspecto `@AuthScope` que inyecta filtros en las queries
...
#### <a id="d12"></a>D12 — Errores de autorización como `Result.Forbidden`
...
#### <a id="d13"></a>D13 — ArchUnit obligatorio: todo `@ApplicationService` autoriza
...
#### <a id="d14"></a>D14 — Tests de acceso cruzado obligatorios por caso de uso
...
#### <a id="d15"></a>D15 — Alcance: denegaciones siempre, accesos a salud y perfil personal
...
#### <a id="d16"></a>D16 — Emisión asíncrona vía evento + outbox
...
#### <a id="d17"></a>D17 — Módulo `auditoria` dedicado, esquema propio
...
#### <a id="d18"></a>D18 — Endpoint `GET /me/permissions` como ayuda de UX, no barrera
...
#### <a id="d19"></a>D19 — Sin rol de soporte interno en MVP (aplazamiento consciente)
...
- **Pérdida de auditoría por fallo de Postmark/Postgres** → la auditoría asíncrona se entrega via outbox con la misma garantía at-least-once del resto (ADR-0007).
