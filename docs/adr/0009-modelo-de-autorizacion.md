...
- **Estado**: Aceptado
- **Fecha**: 2026-05-22 · revisado 2026-05-29 (reorganización Nivel 1: premisas heredadas, NFRs propios, sub-decisiones numeradas D1-D19 con anchors; incorporación de: política frente a proyección stale, patrón de listados con aspecto, errores como `Result.Forbidden`, garantía arquitectónica con ArchUnit, alcance concreto de auditoría, módulo `auditoria` dedicado, endpoint `/me/permissions` como ayuda de UX, decisión consciente de aplazar el rol de soporte interno) · **aceptado 2026-05-29** · revisado 2026-06-12 (**D2**: anotaciones propias `@Authorize`/`@NoAuthRequired` evaluadas contra la `AuthorizationMatrix` en lugar de `@PreAuthorize` de Spring Security — alinea el ADR con la implementación H0 y elimina el SpEL; nota de SpEL en Notas sustituida; citas corregidas: ADR-0004 D7→D4 (esquema por módulo), ADR-0011 ya Aceptado; sin cambio en el modelo de tres capas) · revisado 2026-07-05 (**D11**: de "aspecto que inyecta predicados en la query" a **filtro por firma + aspecto verificador fail-closed** — el método `@AuthScope(CLUB)` recibe `clubId` y filtra en su propia query; el aspecto `AuthScopeEnforcementAspect` verifica en runtime que ese `clubId` coincide con `principal.clubId` y falla cerrado si no hay principal, falta el parámetro o no coincide; scopes sin verificación implementada (`OWNED`, `GRUPOS_DEL_ENTRENADOR`, `MIS_GRUPOS`) fallan cerrado hasta implementarse; ArchUnit exige el parámetro `clubId: UUID` en todo `@AuthScope(CLUB)` — corrige divergencia doc↔código detectada en revisión 2026-07-03; **D13**: añadida opción (d) `@AuthenticatedOnly(justificacion)` para casos de uso/handlers que requieren sesión activa sin regla de matriz aplicable (`QueryCurrentSession`/`SessionController.current()`); sin cambio en el modelo de tres capas) · **revisado 2026-07-11** (**D12**: la premisa heredada de ADR-0008 D11/D12 cambió de `Result<T, DomainError>` a `Either<XxxError, T>` — D12 de este ADR se reescribe (`Either.Left(XxxError.Forbidden)`, `data object` sin parámetro de razón, verificado contra `IdentidadError.kt`); D9 y D14/D15 actualizan sus citas en cascada. `ProjectionStale` queda como variante convenida para módulos consumidores de proyecciones (`identidad` no la necesita — solo publica eventos). Sin cambio de código) · **revisado 2026-09-23** (**D7**: de "`AutorizacionService` por módulo" a **matriz consultada en el caso de uso + reglas de relación en puertos de consulta del módulo** — alinea el ADR con la implementación H0/H1, que nunca tuvo `AutorizacionService`, y con D12, que ya lo negaba desde la revisión del 2026-07-11; D3, D13 (b), D16 y Riesgos actualizan su redacción en cascada; D12 corrige la cita `AccesoDenegado` D9→D15/D16; la emisión de `AccesoDenegado` en toda denegación queda explícita como invariante. Sin cambio de código ni en el modelo de tres capas) · **revisado 2026-09-23** (**D14**: añadida la excepción anti-enumeración para objeto de otro club — dentro del mismo club, `Forbidden` auditado como antes; entre clubes, `NotFound` en vez de `Forbidden`, para no abrir un oráculo de existencia con una consulta sin filtro `club_id` que D11 no permite hacer barata; verificado contra `GetGroupDetailQuery` (`clubtaxonomia`), que ya implementaba este criterio sin que el ADR lo recogiera; caso hoy no alcanzable en el MVP mono-club, ADR-0006. Sin cambio de código)
...
- **Topología y reglas (D5-D9)** — autorización por módulo (sin módulo central), núcleo compartido, reglas de relación por módulo consultadas desde el caso de uso, proyecciones locales y política frente a proyección stale.
...
| D7  | [Reglas de relación por módulo, consultadas desde el caso de uso](#d7)             | Operativa    |
...
Cada caso de uso que carga, modifica o devuelve un objeto comprueba, antes de la operación, la regla RBAC contra la `AuthorizationMatrix` y la relación del principal con el objeto a través de los puertos de consulta del módulo (D7). La consulta a la matriz es explícita y verificable arquitectónicamente (D13); la comprobación de relación se cubre con los tests de acceso cruzado (D14).
...
<a id="d7"></a>
### D7 — Reglas de relación por módulo, consultadas desde el caso de uso

Cada caso de uso (`@ApplicationService`) aplica la autorización **inline, al principio de su bloque `either { }`** (Raise DSL, D12), en dos pasos:

1. **RBAC contra la matriz** del núcleo compartido (D6): `ensure(AuthorizationMatrix.can(actor.role, Resource.X, Action.Y)) { XxxError.Forbidden }`.
2. **Nivel de objeto contra la relación del principal**, resuelta por un **puerto de consulta del módulo** en `application/ports/outbound/` e implementado sobre sus proyecciones locales (D8). Hay dos formas, según el caso:
   - **Pregunta explícita** al puerto cuando la relación es entre el principal y un objeto de otro: *"¿este entrenador es responsable de este grupo?"* (`CoachGroupLookup.isCoachOfGroup(clubId, coach, groupId)` en `planificacion`), *"¿este plan pertenece a un grupo del entrenador?"*.
   - **Relación por construcción** cuando el objeto es del propio principal: la consulta se indexa por `actor.userId` / `actor.clubId` y nunca recibe un identificador de dueño desde fuera (*"¿este reporte fue creado por este alumno?"* se responde leyendo solo los reportes del alumno autenticado). Un objeto ajeno no aparece: colapsa en `Forbidden` o en lista vacía, según D10/D12.

Cada regla de relación vive **una sola vez** en su puerto; los casos de uso que la necesitan reutilizan el mismo método (en `planificacion`, `isCoachOfGroup` lo comparten `AddSessionCommand`, `UpdateSessionCommand`, `PublishPlanCommand`…). Así se mantiene el objetivo original de esta sub-decisión: no duplicar reglas dentro del módulo y poder testearlas en un solo sitio, porque el puerto se sustituye por un doble en los tests de caso de uso y se prueba contra PostgreSQL en su adaptador.

**No existe un `AutorizacionService` por módulo** que envuelva matriz y relaciones (la redacción original de este D7 lo prescribía; ver Notas, revisión del 2026-09-23). Razones:

- **Una fachada sin lógica propia**: la regla RBAC es una llamada de una línea a la matriz y la regla de relación ya vive en su puerto de consulta. Un servicio intermedio solo reexpondría ambas bajo otro nombre, con un bean y un doble de test más por módulo.
- **La verificación estructural no depende de él**: D13 comprueba que cada `@ApplicationService` accede a la `AuthorizationMatrix`, lo que es verificable con ArchUnit sin un tipo intermedio.
- **El motivo de la denegación queda junto a la guarda**: cada `ensure` conoce qué comprobación falló y publica `AccesoDenegado` con ese motivo (`RBAC`, `NotCoachOfGroup`, `ProjectionStale(...)`…), sin tener que reconstruirlo fuera.

**Invariante de auditoría**: toda rama que devuelve `XxxError.Forbidden` (y `ProjectionStale`, D9) publica `AccesoDenegado` dentro de la guarda que deniega, en la misma transacción que el caso de uso (D15, D16). Cómo lo publica cada módulo es detalle de implementación (un componente auditor del módulo o una función privada del caso de uso); lo obligatorio es que **ninguna denegación salga sin su evento**. Hoy este invariante **no lo verifica ArchUnit** (D13 solo exige la consulta a la matriz): lo sostienen la revisión de PR y los tests del caso de uso.
...
- **Cada método público de una clase `@ApplicationService`** tiene **al menos uno** de: (a) anotación `@Authorize(...)`, (b) consulta explícita a la `AuthorizationMatrix` (D7; verificada a nivel de clase: la clase, o alguna de sus clases anidadas, accede a la `AuthorizationMatrix`), (c) anotación `@NoAuthRequired` con justificación en comentario, (d) anotación `@NoAuthRequired`/`@AuthenticatedOnly(justificacion)` a nivel de clase.
...
**Excepción anti-enumeración — objeto de otro club.** La regla de arriba describe el caso dentro del **mismo club**: el objeto se carga con el filtro `club_id` habitual (D4) y, si el llamador no tiene la relación requerida (D3), el caso de uso puede devolver `Forbidden` auditado (D15) sin más coste — no hay ninguna consulta adicional que evitar.

El caso de un objeto de **otro club** es distinto: distinguir "existe pero es ajeno" de "no existe" exige una consulta sin el filtro `club_id`, lo que choca con `@AuthScope(CLUB)` (D11). Para ese caso, el repositorio sigue devolviendo `null` con el filtro de club puesto, y el caso de uso lo traduce a `NotFound`, no a `Forbidden`: no se abre un oráculo de existencia entre clubes a cambio de una consulta que D11 no permite hacer barata. Ejemplo verificado: `GetGroupDetailQuery` (`clubtaxonomia`) — un grupo inexistente y uno de otro club dan el mismo `GroupNotFound`. En el MVP mono-club ([ADR-0006](0006-infraestructura-mono-tenant.md), contexto y D22) este caso no es alcanzable en producción; se reabre al activar el multi-club ([ADR-0006 D16](0006-infraestructura-mono-tenant.md#d16)).
...
La capa de autorización emite un evento de dominio (`AccesoDenegado`, `AccesoADatosSensibles`) en la **misma transacción** que la operación de negocio. `AccesoDenegado` lo publica el propio caso de uso desde la guarda que deniega (D7); `AccesoADatosSensibles`, el aspecto de `@AuditAccess` sobre el método del caso de uso. El outbox de Spring Modulith (ADR-0007 D6) lo entrega al módulo `auditoria` (D17).
...
- **Comprobación a nivel de objeto olvidada en un caso de uso** → ArchUnit (D13, exige la consulta a la matriz) + reglas de relación centralizadas en los puertos de consulta del módulo (D7) + tests de acceso cruzado por caso de uso (D14).
- **Denegación que devuelve `Forbidden` sin publicar `AccesoDenegado`** (se rompe el invariante de D7 y el alcance de D15) → revisión de PR + tests del caso de uso que comprueban la publicación del evento; ArchUnit (D13) **no** lo detecta hoy — candidato a regla o test transversal propio.
...
- **Revisión del 2026-07-05 (D11, D13)**: la revisión multi-agente de arquitectura/seguridad (2026-07-03) detectó que D11 describía un aspecto que **inyectaba** predicados de filtro en la query, pero la implementación real de H0 (`UserRepositoryImpl` y equivalentes) pasa `clubId` como parámetro explícito y filtra en la propia query — no existía ningún `@Aspect`/`@Around` en el backend. En vez de construir el aspecto original (mayor coste, magia oculta sobre Hibernate/queries nativas), se opta por un **aspecto verificador**: el filtro sigue siendo responsabilidad de la firma del método (D4), y el aspecto (`AuthScopeEnforcementAspect`) comprueba en runtime, fail-closed, que el `clubId` recibido coincide con el del principal — cierra la brecha de "un método que no pasa `clubId` no está protegido por nada" sin reescribir el patrón de acceso a datos ya en producción. D13 gana la opción (d) `@AuthenticatedOnly`, que hacía falta para `QueryCurrentSession`/`SessionController.current()` (ni autorizan contra la matriz ni son endpoints públicos) — sin ella, D13 era irrealizable sin mentir sobre esos dos casos.
- **Revisión del 2026-09-23 (D7)**: D7 prescribía un `AutorizacionService` por módulo (puerto en `domain`, implementación en `application`) al que el caso de uso delegaba toda la autorización. La implementación de H0/H1 nunca lo creó: los casos de uso `@ApplicationService` consultan la `AuthorizationMatrix` inline con `ensure(...)` y resuelven la relación del principal con el objeto mediante puertos de consulta del módulo (`CoachGroupLookup`, lectores indexados por el principal); `AuthorizationArchTest` verifica el acceso a la matriz, no un servicio. El propio D12 ya lo recogía desde la revisión del 2026-07-11 ("no hay un `AutorizacionService` separado"), así que el ADR se contradecía internamente. Se reescribe D7 describiendo el mecanismo real; su objetivo (reglas de relación sin duplicar y testeables en un solo sitio) se mantiene, porque cada regla vive una vez en su puerto. Se explicita como invariante que toda denegación publica `AccesoDenegado` (D15/D16), que antes solo se deducía de D15, y se añade el riesgo correspondiente. Cambios en cascada: D3, D13 (b), D16 y Riesgos; D12 corrige la cita `AccesoDenegado` D9→D15/D16. Sin cambio de código ni en el modelo de tres capas (D1).
