...
/** La semana del propio alumno, ya resuelta (alias del modelo generado). */
...
/** Lo que el alumno envía al reportar una sesión (alias del modelo generado). */
export type SessionReportData = MiReporteRequest;

/** Lo que el alumno envía al reajustar el día de una sesión (alias del modelo generado). */
export type RescheduleData = MiReajusteRequest;

/** El reajuste ya guardado de una sesión (alias del modelo generado). */
...
export class MyPlanService {
  ...
  /** Envío idempotente: la primera vez crea el reporte del día, las siguientes lo
  ...
  /** Envío idempotente: la primera vez crea el reajuste del día, las siguientes lo
  ...
  /** Idempotente: deshace el reajuste de [dia], con o sin reajuste previo. */
