...
/**
 * Rutas raíz, con carga diferida por pantalla. Tres bloques:
 *
 ...
 * 3. El shell de gestión en `path: ''`, que envuelve al resto (ADMIN/ENTRENADOR) con la cabecera y
 *    el menú.
 *
 * `/mi-plan` es una ruta de nivel raíz con path propio, no un segundo `path: ''` hermano del shell
 * de gestión: dos `path: ''` compitiendo ya rompió `authGuard` una vez (ver el bloque 1 de más
 * abajo) — mismo motivo por el que las rutas públicas se insertan con spread y no `loadChildren`.
 */
